...
muscle groups sitting below a productive weekly set count.
"""
...
from hevy_coach.analytics.standards import resolve_lift, score_lift
...
# Below this many working sets per week, a muscle group is likely undertrained.
LOW_WEEKLY_SETS = 8
...
def insights(db: Database, settings: Settings, *, days: int = 180) -> list[Insight]:
    ...
    # 3. Undertrained muscle groups.
    for group in metrics.muscle_group_volume(db, days=28):
        if group["sets_per_week"] < LOW_WEEKLY_SETS and group["muscle_group"] not in {
            "cardio",
            "other",
            "full_body",
        }:
            found.append(
                Insight(
                    ...
                    detail=(
                        f"{group['sets_per_week']} working sets/week over the last 4 weeks. "
                        f"Most hypertrophy guidance puts the productive range above "
                        f"{LOW_WEEKLY_SETS}."
                    ),
                    evidence=group,
                )
